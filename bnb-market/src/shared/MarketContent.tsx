...
export function BnbAgentContent({ chainId, id, signedIn = false, onNeedSignIn = () => undefined, walletRequest = null }: { chainId: BnbChain; id: string; signedIn?: boolean; onNeedSignIn?: () => void; walletRequest?: WalletRequest | null }) {
  ...
  return <div className="space-y-6">
    ...
    {agent ? <>
      <div className={PANEL}><div className="font-mono text-[10px] tracking-widest text-accent">BNB {chainId === 97 ? "TESTNET" : "MAINNET"} SERVICE</div><h2 className="mt-4 font-stencil text-4xl uppercase">{agent.name}</h2><p className="mt-3 max-w-[85ch] font-mono text-sm leading-relaxed text-ink-2">{plainServiceDescription(agent.description)}</p>{agent.id === "2177" && agent.services.some((service) => service.name.toLowerCase().replace(/[-_]/g, "") === "erc8183") ? <div className="mt-5 flex flex-wrap items-end gap-8 border-t border-[color:var(--hairline)] pt-4"><div><p className="font-mono text-[10px] uppercase tracking-widest text-ink-3">PRICE</p><p className="mt-2 font-stencil text-2xl uppercase">0.1 U</p><p className="font-mono text-[10px] uppercase text-ink-3">PER REPORT</p></div><div><p className="font-mono text-[10px] uppercase tracking-widest text-ink-3">LIVE TEST</p><p className="mt-2 font-mono text-[11px] uppercase tracking-widest text-accent">AVAILABLE</p></div><a className={`${BUTTON} bg-accent !text-accent-ink`} href="#hire-agent-heading">USE NOW →</a></div> : null}</div>
      <div className="grid gap-4 lg:grid-cols-2">
        <div className={PANEL}><h3 className="font-stencil text-2xl uppercase">WHAT YOU GET</h3><p className="mt-4 font-mono text-[12px] leading-relaxed text-ink-2">A focused service for the outcome shown above. Try the live service first, then decide whether to use the paid option.</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="border border-[color:var(--hairline)] p-4"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-3">SERVICE TYPE</p><p className="mt-2 font-stencil text-xl uppercase">{agent.category ? CATEGORIES.find((c) => c.id === agent.category)?.label : "Analysis service"}</p></div><div className="border border-[color:var(--hairline)] p-4"><p className="font-mono text-[10px] uppercase tracking-widest text-ink-3">NETWORK</p><p className="mt-2 font-stencil text-xl uppercase">BNB {chainId === 97 ? "TESTNET" : "MAINNET"}</p></div></div></div>
        <div className={PANEL}><h3 className="font-stencil text-2xl uppercase">TRY IT LIVE</h3><p className="mt-4 font-mono text-[12px] leading-relaxed text-ink-2">Run a free live check before connecting your wallet. This checks that the service responds; it does not start paid work or move funds.</p><button className={`${BUTTON} mt-5`} disabled={testing || !agent.services.some((s) => s.name.toLowerCase() === "a2a" || (s.name.toLowerCase() === "erc-8183" && new URL(s.endpoint).pathname.endsWith("/status"))) || agent.registrationMatches === false} onClick={testEndpoint}>{testing ? "TESTING SERVICE…" : "TEST SERVICE →"}</button>
          {proof ? <div role="status" className="mt-5 border-l-2 border-accent pl-4 font-mono text-[12px] leading-relaxed text-ink-2"><p className="uppercase text-ink">{proof.status === "reachable" ? "SERVICE RESPONDED" : "SERVICE NEEDS ATTENTION"}</p><p>{proof.message}</p><p className="mt-2">Checked {new Date(proof.checkedAt).toLocaleString()}</p></div> : null}
          <p className="mt-5 font-mono text-[11px] leading-relaxed text-ink-3">Live testing is a quick availability check. Results do not promise a particular outcome.</p>
        </div>
      </div>
      <section className={PANEL} aria-labelledby="outcome-heading"><h3 id="outcome-heading" className="font-stencil text-2xl uppercase">WHAT IT DOES</h3><p className="mt-3 font-mono text-[12px] leading-relaxed text-ink-2">This service is listed for the outcomes below. Read the description and test the service before deciding to use it.</p><div className="mt-5 grid gap-3 md:grid-cols-2">{agent.outcomeMatches.length ? agent.outcomeMatches.map((match) => <div className="border border-[color:var(--hairline)] p-4" key={match.category}><p className="font-stencil text-xl uppercase">{CATEGORIES.find((c) => c.id === match.category)?.label}</p><p className="mt-2 font-mono text-[12px] leading-relaxed text-ink-2">{match.reason}</p></div>) : <p className="font-mono text-[12px] text-ink-2">The provider has not added a category yet.</p>}</div></section>
      {agent.id === "2177" && agent.services.some((service) => service.name.toLowerCase().replace(/[-_]/g, "") === "erc8183") ? <LpHiringPanel key={`${chainId}:${id}`} chainId={chainId} signedIn={signedIn} onNeedSignIn={onNeedSignIn} walletRequest={walletRequest} /> : <CommerceReadinessPanel key={`${chainId}:${id}`} chainId={chainId} agentId={id} />}
      <details className={PANEL}><summary className="cursor-pointer font-mono text-[12px] uppercase tracking-widest">MORE DETAILS</summary><div className="mt-5 space-y-6 break-all font-mono text-[12px] text-ink-2"><div><p className="text-ink-3">OWNER</p><p className="mt-2"><a className="underline underline-offset-4" href={`${explorer}/address/${agent.owner}`} target="_blank" rel="noopener noreferrer">{agent.owner} ↗</a></p></div><div><p className="text-ink-3">AGENT WALLET</p><p className="mt-2">{agent.wallet}</p></div><div><p className="text-ink-3">CHECKED AT</p><p className="mt-2"><a className="underline" href={`${explorer}/block/${agent.blockNumber}`} target="_blank" rel="noopener noreferrer">BLOCK {agent.blockNumber} ↗</a> · {new Date(agent.checkedAt).toLocaleString()}</p></div><div><p className="text-ink-3">REGISTRATION</p><p className="mt-2">{agent.metadataStatus === "available" ? agent.registrationMatches === false ? "Network or identity mismatch. Do not use this registration." : "Readable provider metadata; service claims are not independently verified." : "Metadata unavailable. Identity is registered, but service details could not be read."}</p></div><div><p className="text-ink-3">REGISTRY</p><p className="mt-2">{agent.registry}</p></div><div><p className="text-ink-3">METADATA SNAPSHOT HASH</p><p className="mt-2">{agent.versionHash ?? "Unavailable"}</p></div><div><p className="text-ink-3">SERVICE CONNECTIONS</p>{agent.services.length ? agent.services.map((s) => <p className="mt-2" key={`${s.name}:${s.endpoint}`}>{s.name}: {s.endpoint}</p>) : <p className="mt-2">No supported public HTTPS service endpoints.</p>}</div><div><p className="text-ink-3">SERVICE CHECKS</p>{(agent.capabilities.length ? agent.capabilities : deriveMarketCapabilities(agent.services)).map((capability) => <p className="mt-2" key={capability.protocol}>{capability.protocol} · {capability.state} · {capability.reason} · {capability.endpoint}</p>)}</div></div></details>
    </> : null}
  </div>;
}
