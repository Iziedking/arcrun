...
function Content({ view }: { view: "market" | "detail" | "publish" | "playground" }) {
  ...
  return <div className="min-h-screen bg-canvas text-ink">
    <header className="border-b border-[color:var(--hairline)]"><div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6">
      ...
      <main className="mx-auto max-w-[1600px] px-4 pb-20 pt-10 sm:px-6 sm:pt-12">{view !== "detail" ? <section className="flex flex-wrap items-end justify-between gap-6"><div><p className="font-mono text-[11px] uppercase tracking-widest"><span className="text-accent">■</span> AGON MARKET / BNB {chainId === 97 ? "TESTNET" : "MAINNET"}</p><h1 className="mt-4 font-stencil uppercase leading-none" style={{ fontSize: "clamp(40px,5vw,72px)" }}>{view === "market" ? "FIND AN AGENT" : view === "playground" ? "TRY AN AGENT" : "LIST YOUR AGENT"}</h1><p className="mt-4 max-w-[56ch] font-mono text-sm leading-relaxed text-ink-2">{view === "market" ? "Browse services, compare prices, and choose one to try." : view === "playground" ? "Try a service before connecting your wallet." : "List a service so buyers can discover and use it."}</p></div><a className={BUTTON} href={bnbHref(chainId, view === "market" ? "/market/new" : "/market")}>{view === "market" ? "LIST YOUR AGENT →" : "BACK TO MARKET →"}</a></section> : <div className="mb-6"><a className="font-mono text-[11px] uppercase tracking-widest text-ink-2 underline underline-offset-4" href={bnbHref(chainId, "/market")}>← BACK TO MARKET</a></div>}
      <section className={view === "detail" ? "mt-0" : "mt-12"}>{view === "playground" ? <BnbPlaygroundContent key={chainId} chainId={chainId}/> : view === "market" ? <BnbMarketContent key={chainId} chainId={chainId}/> : view === "detail" ? <BnbAgentContent key={`${chainId}:${params.id}`} chainId={chainId} id={params.id ?? ""} signedIn={!!activeSession} onNeedSignIn={() => setLoginOpen(true)} walletRequest={async (input: Parameters<WalletRequest>[0]) => {
        const wallet = (window as Window & { ethereum?: Wallet }).ethereum;
        if (!wallet) throw new Error("Open AGON in your wallet browser or install a browser wallet to continue.");
